...
| `AP_ENFORCE_CONNECTION_PIECE_BINDING` | Restrict every connection to the piece that created it, and block connection access entirely for pieces with no fixed destination (HTTP, GraphQL, Webhook, and self-hosted/DB-style pieces). Applies platform-wide and immediately to existing connections/flows — including making the "Read Connection" action non-functional. | `false` |
